...
def horizontal_fov():
    ...
    midpoint.z = camera.location.z
    
    # Calculate the position of a hypothetical camera positioned orthoginally to the target frame
    offset_camera_position = Vector(camera.location)
    offset_camera_position.x = midpoint.x
    ...
    fov_in_mm = camera.data.sensor_width / (2.0 * math.tan(fov_angle / 2.0))
...
def get_horizontal_shift():
    ...
    camera_center_global_offset = center.x - camera.location.x
